...
from app.apis.category.v1.home_category_router import CoordinatesRequest
...
async def test_api_get_categories_distinct() -> None:
    ...
    assert [code for code in response.json()["categories"]] == [CategoryCode.CHICKEN.value]


async def test_api_get_categories_distinct_latitude_too_big() -> None:
    # Given
    too_big_latitude = 47.49006

    # When
    async with AsyncClient(app=app, base_url="http://test") as client:
        response = await client.get(f"/v1/home_categories/distinct?longitude=127.0056&latitude={too_big_latitude}")
